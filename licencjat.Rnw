...
Prezentujemy wspólny wykres. Kolor czerowny odpowiada przybliżeniu uzyskanemu ze schematu RK4 dla $N=25$.
...
Zaprezentujemy teraz fragment wykresu pokazujący róznice uzyskane przez przybliżenia dla $N=25$, $N=50$, $N=100$. 
<<fig=TRUE>>=
t = seq (t0, to= 4*pi, by=0.01)
...
\section{Problemy brzegowe}
\begin{theorem}[O istenienu rozwiązania \citep{cheney2006analiza}]
Jeśli funckja $u'(x)$ jest ciągła, nieujemna i ograniczona w obszarze określonym nierównościami $0\leq t \leq 1$, $-\infty < x < +\infty$ to problem brzegowy postaci:
...
Rozważmy następujące problemy brzegowe:
...
$$
Jeśli $y$ jest rozwiązaniem \eqref{drugi_problem_brzegowy}, wówczas funkcja $u$ postaci $u(t)=y\left(\frac{t-a}{b-a}\right)$ jest rozwiązaniem problemu \eqref{pierwszy_problem_brzegowy}. Ponadto, jeśli $u$ jest rozwiązaniem \eqref{pierwszy_problem_brzegowy}, wówczas $y(a+(b-a)t)$ jest rozwiązaniem problemu \eqref{drugi_problem_brzegowy}.
\end{theorem}
\begin{proof}\citep{kincaid1991numerical}
Dowód powyższego twierdzenia wygląda w następujący sposób:
$$
\begin{array}{rl}
...
$$
Jeśli $z$ jest rozwiązaniem \eqref{czwarty_problem_brzegowy}, to funkcja $y(t) = z(t) + \alpha + (\beta - \alpha)t$ jest rozwiązaniem \eqref{trzeci_problem_brzegowy}. Co więcej jeżeli $y$ jest rozwiązaniem \eqref{trzeci_problem_brzegowy}, to $z(t) = y(t) - [\alpha + (\beta - \alpha)t]$ jest rozwiązaniem \eqref{czwarty_problem_brzegowy}.
\end{theorem}
\begin{proof}
Dowód tego twierdzenia wynika bezpośrednio z:
$$
\begin{array}{rl}
...
\begin{theorem}\citep{kincaid1991numerical}
Jeżeli $f(t,s(t))$ jest funkcją ciągłą, gdzie $t \in [0,1]$ i $(-\infty,+\infty)$ spełniającą warunek taki, że
$$
...
W tej częsci przedstawimy jeden ze schematów rozwiązywania problemu brzegowego postaci:
...
\begin{enumerate}
\item Zamieniamy problem brzegowy, na zagadnienie początkowe dowolnie dobierając wartość $u'(a)$. 
\item Rozwiązujemy powstałe zagadnienie początkowe uzyskując wartość $u(b)$.
\item Jeśli dobór wartośći $u'(a)$ z punktu (1) był odpowiedni to $u(b) = \beta$, co kończy obliczenia. Jeżeli $u(b) \neq \beta$ należy wrócić do punktu (1), ponownie wybrać (tym razem oczywiście inne u'(a)) i przejść do punktu (2) oraz (3). 
\end{enumerate}
\end{algorithm}
\subsection{Metoda różnic skończonych}
Inny sposób rozwiązywania problemów brzegowych polega na zastąpieniu pochodnych wyrażeniami przybliżonymi i dyskretyzacji dziedziny. Posługiwać będziemy się wzorami:
\begin{equation}
...
\left\{\begin{array}{ll}
y_0 = \alpha, & \\
\frac{1}{h^2}(y_{n-1}-2y_n + y_{n+1} = f\left(t_n,y_n,\frac{1}{2h}(y_{n+1} - y_n)\right),& n \in \set{1,\cdots,N} \\
y_{n+1} = \beta.
\end{array}\right. 
$$
powyższy układ $N$ równań z $N$ niewiadomymi $y_1,\cdots,y_N$ jest nieliniowy. Rozwiązywanie ukłądu w tej postaci jest kłopotliwe, znacznie łatwiej jest go rozwiązać w przypadku liniowym. Rozważmy zatem równanie:
$$
f(t,x(t),x'(t)) = u(t) + v(t)x + w(t)x',
$$
wówczas mamy układ:
$$
\left\{\begin{array}{ll}
y_0 = \alpha, & \\
\left(1+ \frac{1}{2}hw_n\right)y_{n-1} - (2+ h^2v_n)y_n + (1- \frac{1}{2}hw_n)y_{n+1} = h^2w_n ,& n \in \set{1,\cdots,N} \\
y_{n+1} = \beta,
\end{array}\right. 
$$
gdzie $u_n := u(t_n)$. Wprowadzmy oznaczenia :
$ a_n:= 1 + \frac{1}{2}w_{n+1}, d_n:= -(2+h^2v_n), c_n:= 1 - \frac{1}{2}hw_n, b_n:= h^2u_n $
i zapiszmy powyższy układ w postaci macierzowej:
...
\end{array} \right] .
\end{equation}
Macierz układu jest trójprzekątniowa, co pozwala rozwiązać go za pomocą spejalnej wersji eliminacji Gaussa. Zauważmy też, że jeśli $v(t)>0$, a $h$ jest na tyle małe, że$ |hw_n| \leq 2$, to ta macierz jest przekątniowo dominująca. Istotnie, wtedy $ 1 \pm \frac{1}{2}hw_n \geq 0 $ i
$$
|2 + h^2v_n| > |1 + \frac{1}{2}hw_n| + |1 - \frac{1}{2}h^2w_n| = 2
$$
W dalszej częsci będziemy korzystać z równości:
\begin{equation}
|d_n| - |c_n| - |a_{n-1}| = h^2v_n,
\end{equation}
poprawnej przy powyższych założeniach o $v$ i $h$. 
\subsection{Zbieżność}
W tej seksji wykażemy, że dla $h \to 0$ rozwiązanie zadania dyskretnego jest zbieżne do rozwiązania problemu brzegowego postaci
...
\end{array}\right.
\end{equation}
Istnienie i jednoznaczność rozwiązania tego zagadnienia wynika z następującego twierdzenia.
\begin{theorem}\citep{cheney2006analiza}
...
\begin{enumerate}
\item Funkcja $f$ i jej pierwsze pochodne cząstkowe $f_t$,$f_x$,$f_{x'}$ są ciągłe w obszarze $D:= [a,b] \times \setR^2$.
\item $f_x >0, f_x \leq M$ i $|f_{x'}| \leq M$ w $D$.
...
$$
Odejmujemy stronami odpowiednie równania z obu układów, przyjmując oznaczenie $e_1:=x_n - y_n$
\begin{equation}
