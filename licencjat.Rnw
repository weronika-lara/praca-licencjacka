...
\section{Przykłady numeryczne}
W sekcji tej zaprezentujemy przykłady wykorzytsujące schemat Eulera otwarty(w skrócie będziemy pisali schemat Eulera) oraz Rungego-Kutty rzędu 4 (w skórcie będziemy oznaczali RK 4). Weźmy równanie różniczkowe postaci:
$$
x'' = -x \quad x(0)=0, x'(0) = 1.
$$
Jego rozwiązaniem jest oczywiście funkcja $\sin(t)$. Zdefiniujmy teraz kody odpowiadające naszym schematom. Potrzebne będzie 5 parametrów:
\begin{itemize}
...
\end{itemize}
Zacznijmy od schematu Eulera. Przypomnijmy wzór na dla tego algorytmu:
$$
...
@
Przypomnijmy wzór na schemat otwarty RK 4
$$
x(t_{n}) = x(t_{n-1}) + \frac{h}{6} ( K_1 + 2K_2 + 2K_3 + K_4 )
$$
gdzie 
...
@
Przedstawimy wspólny wykres funkcji $\sin(t)$ porównujący te dwa schematy. Kolor czerowny odpowiada przybliżeniu otrzymanego przez schemat Eulera, zielony obrazuje przybliżenie otrzymane z RK 4.
<<fig=TRUE>>=
...
@
W dalszej częsci przykładu pokazujemy jak istotny jest dobór odpowiedniego $N$. Różnice pokażemy dla funkcji $sin(t)$ i na schemacie RK 4. 
\begin{itemize}
\item $N=10000$,
\item $N=1000$,
\item $N=100$.
...
Prezentujemy wspólny wykres. Kolor czerowny odpowiada przybliżeniu uzyskanemu ze schematu RK 4
...
\end{problem}
\chapter{Przykładowe rozwiązanie schematu brzegowego jednowymiarowego}
W dalszej części zainteresowaliśmy przejściem od zagadnień początkowych, do trudniejszych zagadnień - tj. zagadnień brzegowych. W tym eksperymencie przyjrzymy się liniowemu równaniu drugiego stopnia niejednorodnym z warunkami brzegowymi w przypadku jednowymiarowym. Interesuje nas równanie postaci
$$
\left\lbrace
\begin{array}{c}
