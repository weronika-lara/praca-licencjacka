...
\item $\delta \Omega $ brzeg zbioru.
\item $J$ dowolny przedział.
%\item $\operatorname{IntB}$ wnętrze zbioru.
\item $\ball{(t_0,x_0)}{\delta}$ kula o promieniu $\delta$ w punkcie $(t_0,x_0)$.
...
\begin{definition}[Operator różnicowy symetryczny \citep{marcinkowski2011numeryczne}]
...
Powyższa postać równania różniczkowego nie jest jednak najpowszechniej spotykaną. Powyższą postać można by utożsamiać z postacią uwikłaną funkcji. Częściej natomiast interesują nas równania różniczkowe zadane w postaci jawnej.
...
\end{array} \right.
\end{equation}
gdzie $ s$ i $v$ to ustalone wartości. Rozwiazaniem tego zagadnienia nazwiemy funkcję $x$ klasy $C^2(t_0,b) \cap C^1([t_0,b))$ spełniającą powyższe warunki.
\end{problem}
...
\end{array} \right. 
\end{equation}
nazywamy równaniem Poissona, a $u$ nazywamy jego rozwiązaniem.
\end{problem}
...
\chapter{Numeryczne zagadnienia początkowe} \label{chapter-Cauchy-problems}
...
W sekcji tej zaprezentujemy przykłady wykorzystujące schemat Eulera otwarty(w skrócie będziemy pisali schemat Eulera) oraz Rungego-Kutty rzędu 4 (w skrócie będziemy oznaczali RK4). Weźmy równanie różniczkowe postaci:
$$
...
\end{itemize}
\begin{example}\label{example-otwarty-euler}
...
<<>>=
solution2 = RK4_otwarty(rownanie1, x0, t0, h , N)
@

Przedstawimy wspólny wykres funkcji $\sin(t)$ porównujący te dwa schematy. Kolor czerwony odpowiada przybliżeniu otrzymanego przez schemat Eulera, zielony obrazuje przybliżenie otrzymane z RK4.
<<fig=TRUE>>=
...
Patrząc na zaprezentowane przez nas wyżej wzory definiujące odpowiednie schematy, można było przypuszczać, który ze schematów okaże się dokładniejszy. Wykres pokazuje, że schemat RK4, przez to że ma wzór złożony z większej ilości składowych, daje nam lepsze przybliżenie niż prostszy schemat Eulera.
\end{example}
\begin{example}
W tej części pokażemy jak istotny jest dobór odpowiedniego $N$. Różnice zaprezentujemy dla funkcji $sin(t)$ przybliżając ją schematem RK4. 
\begin{itemize}
\item $N=25$,
\item $N=50$,
\item $N=100$.
...
N = 25
h = (t_end - t0) / N
solution2_1 = RK4_otwarty(rownanie1, x0, t0, h , N)
@

Prezentujemy wspólny wykres. Kolor czerwony odpowiada przybliżeniu uzyskanemu ze schematu RK4 dla $N=25$.
...
N = 50
h = (t_end - t0) / N
solution2_2 = RK4_otwarty(rownanie1, x0, t0, h , N)
@

Generujemy wykres. Kolor zielony odpowiada przybliżeniu uzyskanemu ze schematu RK4, dla $N=50$.
<<fig=TRUE>>=
t = seq (t0, to= 4*pi, by=0.01)
plot(t, sin(t), type = 'l', col = 'black')
...
N = 100
h = (t_end - t0) / N
solution2_3 = RK4_otwarty(rownanie1, x0, t0, h , N)
@
...
Zaprezentujemy teraz fragment wykresu pokazujący różnice uzyskane przez przybliżenia dla $N=25$, $N=50$, $N=100$. 
<<fig=TRUE>>=
t = seq (t0, to= 4*pi, by=0.01)
plot(t, sin(t), type = 'l', col = 'black', ylim=c(0.95,1.0001), xlim=c(7.5,8.5))
lines(solution2_1$time, solution2_1$values[1,], col='red')
lines(solution2_2$time, solution2_2$values[1,], col='green')
lines(solution2_3$time, solution2_3$values[1,], col='purple')
@
...
W tym rozdziale odmiennie niż w rozdziałach poprzednich posługiwać się  będziemy literą $u$ do oznaczania poszukiwanej funkcji. Zmienne w równaniu oznaczać natomiast będziemy $t$,$x$,$y$. Ponadto używać będziemy zamiennie następujące oznaczenia dla pochodnych w równaniu:
\begin{itemize}
\item pierwszą pochodną danej funkcji $\ddx{x} u(x) $ zamiennie będziemy oznaczali przez $u_{x}$ lub $u'$ gdy nie ma wątpliwości co do zmiennej po której różniczkujemy,
\item drugą pochodną danej funkcji $\ddx[^2]{x} u(x)$ zamiennie będziemy oznaczali przez $u_{xx}$ lub $u''$ gdy nie ma wątpliwości co do zmiennej po której różniczkujemy.
\end{itemize}
Oznaczenia przyjęte w tej części są typowe w teorii równań cząstkowych. 

\section{Zagadnienia brzegowe dla równań zwyczajnych}
\input{chapters/chapter4.1}

%\section{Schemat brzegowy dla problemu jednowymiarowego}
%\input{chapter/chapter4.intro.2}
...
\right.
$$
Do rozwiązania tego zagadnienia wykorzystamy następujące przybliżenie operatora drugiej pochodnej. Dokładnie skorzystamy z operatorów 
$$
...
$$
Zauważmy, że jest to szczególny przypadek rozważany w sekcji \ref{subsection-boundary-theory-finite-differences} - jednak z uwagi na stałość poszczególnych współczynników jego implementacja jest prostsza. 
\begin{example}
W tym przykładzie konkretnie omówimy równanie drugiego rzędu postaci
$$
u'' = -u \quad u(0)=0, u'(0) = 1.
$$
Jego rozwiązaniem jest funkcja $\sin(t)$. Zdefiniujmy kod dla naszego schematu. Potrzebne będzie 7 parametrów:
\begin{itemize}
\item Ilość kroków $N$.
...
Przedstawimy wykres funkcji $sin(t)$ wraz z przybliżeniem otrzymanym z algorytmu. Przybliżenie otrzymane przy użyciu schematu brzegowego jednowymiarowego oznaczone jest kolorem czerwonym.
<<fig=TRUE>>=
...
\section{Implementacja pełnego schematu brzegowego}

W tej sekcji przedstawimy rozwiązanie wykorzystujące już pełną implementację schematu omówionego w sekcji \ref{subsection-boundary-theory-finite-differences}
...
$$
Rozwiązanie teoretyczne jest postaci:
$$
u(t) = 7\sin(t) + 3\cos(t)
$$
Zdefiniujmy funkcje $f$,$g$,$m$
<<>>=
f = function(t){
  return(rep(0,times = length(t)))
}
...
xN = 7
N =100
solution1 = schemat_brzegowy_pelny(f,g,m,t0 , tN, x0, xN , N)
@

Wygenerujmy wykres:
<<fig=TRUE>>=
t = seq(0,to=pi, length.out = 1000)
plot(t, 7*sin(t)+3*cos(t),type = 'l')
lines(solution1$time, solution1$values,col='red')
@
\end{example}

\begin{example}
Weźmy równanie różnicowe postaci
$$
u''(t) = 2e^t - u(t), \quad u(0) = 2, \quad u(1) = e + \cos(1).
$$
Rozwiązaniem powyższego równania jest funkcja:
$$
u(t) = e^t + \cos(t)
$$
Zdefiniujmy funkcje $f$, $g$ oraz $m$:
<<>>=
f = function(t){
  return(2*exp(t))
}
...
Wygenerujmy wykres:
<<fig=TRUE>>=
t = seq(0,to=1.2, length.out = 1000)
plot(t, exp(t)+cos(t),type = 'l')
lines(solution1$time, solution1$values,col='red')
@
...
u(x_{i,j}) = g(x_{i,j}), \quad i,j \in \set{0,\cdots,N} \land \left( i \in \set{0,N} \lor j \in \set{0,N} \right).
\end{array} \right.
\end{equation}
Zapis tego problemu dla siatki $i,j = \set{0,\cdots,N} $ jest zadaniem trudnym, dlatego algorytm ten zaprezentujemy na konkretnym przykładzie.
\begin{example}
...
Zdefiniujmy funkcję użytą w naszym algorytmie:
<<>>=
f = function(x,y){
  n = length(x)
  m = length(y)
...
Wygenerujmy wykres pokazujący przybliżenie uzyskane po zastosowaniu naszego schematu.
<<fig=TRUE>>=
library(plot3D)
x = solution$x
y = solution$y
