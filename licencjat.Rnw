...
W sekcji tej zaprezentujemy przykłady wykorzytsujące schemat Eulera otwarty(w skrócie będziemy pisali schemat Eulera) oraz Rungego-Kutty rzędu 4 (w skórcie będziemy oznaczali RK4). Weźmy równanie różniczkowe postaci:
$$
...
\end{example}
\end{problem}
\section{Przykładowe rozwiązanie schematu brzegowego jednowymiarowego}
W tym rozdziale zaprezentujemy przykładowe rozwiązanie otrzymane przy wykorzystaniu schematu brzegowego jednowymiarowego. Przyjrzymy się liniowemu równaniu drugiego stopnia niejednorodnemu z warunkami brzegowymi w przypadku jednowymiarowym. Interesuje nas równanie postaci
$$
\left\lbrace
\begin{array}{c}
...
\end{example}
\subsection{Przykładowe rozwiązanie równania eliptycznego przy użyciu schematu brzegowego dwuwymiarowego}
...
Wygereujmy wykres pokzujący nasze bliżenie otrzymane dzięki naszemu algorytmowi:
<<fig=TRUE>>=
library(plot3D)
x = solution$x
y = solution$y
